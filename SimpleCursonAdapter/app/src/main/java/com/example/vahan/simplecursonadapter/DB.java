...
class DB {
    ...
    private static final String DB_NAME = "notegfhjsg2gdgdckc1rdht1";
    ...
    Cursor getData() {
        String sqlQuery = "Select * From mytab Order By  _id";
    ...
    void addRec(String txt,String date, int img) {
    ...
    void editRec(String txt, String date,String id,int img){
        ContentValues cv = new ContentValues();
        cv.put(COLUMN_TXT, txt);
        cv.put(COLUMN_DATE, date);
        cv.put(COLUMN_IMG, img);
    ...
    void editImg(String id,int img){
        ContentValues cv = new ContentValues();
        cv.put(COLUMN_IMG,img);
        cv.put(COLUMN_ID,id);
        mDB.update(DB_TABLE, cv , COLUMN_ID + " = ?" , new String[]{id});
    ...
    private class DBHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            ContentValues cv = new ContentValues();
                cv.put(COLUMN_TXT, "Note example");
                cv.put(COLUMN_DATE,"Creation time...");
                cv.put(COLUMN_IMG, R.drawable.blank);
